  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const receiptResponse = await axios.get('http://localhost:8080/api/warehouse/receipts', {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        setUsers([
          { id: 1, username: 'farmer1', email: 'farmer1@example.com', role: 'FARMER', fullName: 'John Farmer' },
          { id: 2, username: 'lender1', email: 'lender1@example.com', role: 'LENDER', fullName: 'Jane Lender' },
          { id: 3, username: 'admin1', email: 'admin1@example.com', role: 'ADMIN', fullName: 'Bob Admin' }
        ]);
      } catch (error) {
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">Active Receipts</dt>
                    <dd className="text-lg font-medium text-gray-900">{receipts.filter((receipt: any) => receipt.status === 'ACTIVE').length}</dd>
                  </dl>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            <div className="px-4 py-5 sm:px-6">
              ...
              <p className="mt-1 max-w-2xl text-sm text-gray-500">Latest loan applications</p>
            </div>
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            <div className="px-4 py-5 sm:px-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900">Recent Warehouse Receipts</h3>
              <p className="mt-1 max-w-2xl text-sm text-gray-500">Latest warehouse receipts</p>
            </div>
            <div className="border-t border-gray-200">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
